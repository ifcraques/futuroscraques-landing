...
function useCounter(target, duration = 4000, active = false) {
...
const stats = [
  { count: 30000, label: 'Jovens Impactados', suffix: '+' },
  { count: 48, label: 'Projetos Entregues', suffix: '' },
  { count: 30, label: 'Cidades Brasileiras', suffix: '+' },
  { count: 20, label: 'Cidades Internacionais', suffix: '+' },
  { count: 20, label: 'Atletas na Seleção 3x3', suffix: '+' },
]
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    if (statsRef.current) {
      ...
      const alreadyVisible = rect.top < window.innerHeight + 200 && rect.bottom > 0
      if (alreadyVisible) {
        // Já está na tela ou próximo — dispara depois de um pequeno delay
        const timer = setTimeout(() => setCounterActive(true), 400)
